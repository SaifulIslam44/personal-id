    ...
    const fetchProfiles = async () => {
        ...
        const missingFids = uniqueWinnersList
            ...
            .filter(fid => {
                // যদি ক্যাশে থাকে
                if (cachedData[fid] && !winnersProfiles[fid]) {
                    newFromCache[fid] = cachedData[fid]; // স্টেটে ডিরেক্ট না পাঠিয়ে অবজেক্টে রাখছি
                    fetchedFidsRef.current.add(fid); 
                    return false; // ফেচ লিস্ট থেকে বাদ
                }
                // যদি ক্যাশে না থাকে এবং আগেও ফেচ না করে থাকি
...
export default function GiveawayPage(props: any) { 
  ...
  const [showMainList, setShowMainList] = useState(true);
   const [optimisticCount, setOptimisticCount] = useState<number | null>(null);
   const [isWaitingForTx, setIsWaitingForTx] = useState(false); // 🔥 নতুন: ট্রানজেকশনের জন্য অপেক্ষা করার স্টেট
...
// বাটন ডিজেবল করার জন্য যেকোনো একটি Pending অথবা ট্রানজেকশনের জন্য ওয়েট করলে লোডিং দেখাবে
const isClaiming = isClaimingCalls || isClaimingTx || isWaitingForTx;
...
// 🔴 Wagmi Auto Silent Connect for Warpcast
  ...
  // 🔥🔥 FIX: Token Symbol & Decimals for MAIN Card 🔥🔥
const { decimals, tokenSymbol } = useMemo(() => {
    // পুরোনো বেস নেটওয়ার্কের টোকেন লজিক কমেন্ট করে দেওয়া হলো
    /*
    if (activeGiveawayId === 4) return { decimals: 18, tokenSymbol: "$JESSE" };
    if (activeGiveawayId === 25) return { decimals: 18, tokenSymbol: "$BETR" };
    if ([26, 28].includes(activeGiveawayId)) { 
    ...
    */
    ...
    // সেলো নেটওয়ার্কের ডিফল্ট টোকেন রিটার্ন
    return { decimals: 18, tokenSymbol: "$CELO" };
  }, []);
  // [activeGiveawayId]);
...
// --- 🔵 UPDATED FETCH PROFILES (Replace old function) ---
  const fetchProfiles = useCallback(async (winners: {fid: number}[]) => {
    ...
    const missingFids = winners
      .map(w => w.fid)
      .filter(fid => {
        // যদি নিজের প্রোফাইল হয় অথবা ইতিমধ্যে ক্যাশে থাকে
        if (cachedData[fid]) {
          newFromCache[fid] = cachedData[fid];
          fetchedFidsRef.current.add(fid); // মার্ক করে দিলাম যাতে আর ফেচ না হয়
          hasCacheHit = true;
          return false; // লিস্ট থেকে বাদ (ফেচ করার দরকার নেই)
        }
        // যদি আগেও একবার ফেচ করে থাকি (Ref চেক)
    ...
    // ৩. যদি ক্যাশে কিছু পাওয়া যায়, স্টেটে সেট করে দিই
    if (hasCacheHit) {
      setWinnersProfiles(prev => ({ ...prev, ...newFromCache }));
    }

    // ৪. যদি সব ক্যাশে পাওয়া যায়, তাহলে আর নিচে যাওয়ার দরকার নেই
    if (missingFids.length === 0) return;
    ...
    for (const chunk of chunks) {
        try {
            ...
            if (response.ok) {
                ...
                // ডিফল্ট ভ্যালু (যদি API নাম না দেয়)
                chunk.forEach(fid => {
                    if (!newFetchedData[fid]) {
                        newFetchedData[fid] = { 
                            fid, 
...
//         BigInt(giveawayId), 
//         BigInt(userFid), 
//         BigInt(signData.nonce), // 🔥 নতুন প্যারামিটার Nonce যোগ করা হয়েছে
//         signData.signature as `0x${string}` 
//       ],
//     });
...
const onClaim = async () => {
  try {
    ...
    // 🔥 ফিক্স: যদি MiniPay না হয় এবং FID না থাকে, তবেই শুধু এরর দিবে
    if (!giveawayId || (!_isMiniPay && !userFid)) {
      setFarcasterError("Missing user identity or Giveaway ID");
      return;
    }
    ...
    // ৬. সাকসেস হ্যান্ডলিং (🔥 Updated with smart polling)
    if (txId) {
      setIsWaitingForTx(true); // "Sending..." অবস্থা চালু করা হলো

      // প্রতি ২ সেকেন্ড পর পর স্মার্ট কন্ট্রাক্ট চেক করে দেখবে ব্লকচেইন ট্রানজেকশন সফল হয়েছে কিনা
      let attempts = 0;
      const maxAttempts = 30; // সর্বোচ্চ ৬০ সেকেন্ড চেক করবে (30 * 2s)

      const checkConfirmation = async () => {
        try {
          const { data: newStats } = await refetchStats();
          const newClaimCount = Number((newStats as any)?.[1] || 0);

          if (newClaimCount > realCount) {
             // 💥 ট্রানজেকশন ব্লকচেইনে সফল হয়েছে!
             setIsWaitingForTx(false); // "Sending..." অফ করা হলো
             setOptimisticCount(newClaimCount);
             setLastClaimedAmount(rewardAmountFormatted); 
             setShowSuccessModal(true); // সাকসেস পপআপ দেখানো হলো
             
             // নতুন ডেটা রিফ্রেশ করা
             refetchDetails(); 
             refetchWinners(); 
             return; // পোলিং বন্ধ
          }
        ...
        if (attempts >= maxAttempts) {
           setIsWaitingForTx(false);
           setFarcasterError("Transaction confirmation is taking longer than expected. Please check back later.");
           return;
        }

        setTimeout(checkConfirmation, 2000); // ২ সেকেন্ড পর আবার চেক করবে
      };

      checkConfirmation(); // পোলিং শুরু
    }
