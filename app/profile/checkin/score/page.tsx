...
export default function ScorePage() {
  const { context } = useMiniKit();
  // Unused state variable fixed by removing the variable name but keeping the setter if needed
  const [, setFrameContext] = useState<any>(null); 
  const [isDarkMode, setIsDarkMode] = useState(true);
  const [displayScore, setDisplayScore] = useState(0.0);
  ...
  useEffect(() => {
    ...
    let start = 0;
    const duration = 2000;
    const increment = actualScore / (duration / (1000 / 60));
  ...
  return (
    <div className={`${styles.container} ${!isDarkMode ? styles.lightMode : ""}`}>
      ...
      <main className={styles.mainContent}>
        <header className={styles.heroHeader}>
          <h1 className={styles.mainTitle}>NEYNAR SCORE</h1>
          <p className={styles.subTitle}>Reputation score based on your Activity</p>
        </header>
